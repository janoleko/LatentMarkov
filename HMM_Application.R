# This case study illustrates discrete-time HMMs by analyzing the movement track of an elephant from Etosha National Park.
...
for(j in 1:N){
  allprobs[ind, j] = dgamma(data$step[ind], shape = mu[j]^2 / sigma[j]^2, 
...
legend("top", inset = 0, col = c(color, "black"), lty = c(1, 1, 1, 2),
       legend = c("resting", "searching & foraging", "travelling", "marginal distribution"), 
       lwd = 2, horiz = T, bty = "n", text.width = c(0, 0.15, 0))
...
for(j in 1:N){ 
  polygon(c(todseq, rev(todseq)), c(DeltaCI[, j, 1], rev(DeltaCI[, j, 2])), 
          col = alpha(color[j], 0.2), border = F)
