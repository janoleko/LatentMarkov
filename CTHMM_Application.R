...
hist(data$fev, prob = T, border = "white", main = "Histogram of fev", 
     xlab = "fev", breaks = 100)
...
# average number of years it takes to get to next disease stage/ death:
round((1 / Q) / 365, 2)
...
# empirical state frequencies for acute = 0 and acute = 1
(delta0 = c(sum(states[which(data$acute == 0)] == 1), 
           sum(states[which(data$acute == 0)] == 2)) / 
    nrow(data[which(data$acute == 0), ]))
(delta1 = c(sum(states[which(data$acute == 1)] == 1), 
            sum(states[which(data$acute == 1)] == 2)) /
    nrow(data[which(data$acute == 1), ]))
# probability of being in the diseased state is much higher after infection
...
# marginal distribution without and with acute infection in the last 14 days
par(mfrow = c(1, 2))
hist(data$fev[which(data$acute == 0)], prob = T, border = "white", ylab = "density",
     main = "acute = 0", xlab = "fev", breaks = 150, ylim = c(0, 0.02), xlim = c(0, 150))
for(j in 1:2) curve(delta0[j] * dnorm(x, beta[j, 1], sigma[j]), add = T, 
                    col = color[j], lwd = 2)
curve(delta0[1] * dnorm(x, beta[1, 1], sigma[1]) + delta0[2] * dnorm(x, beta[2, 1], sigma[2]),
      add = T, lwd = 2, lty = 2)
legend("topleft", lwd = 2, col = c(color, "black"), lty = c(1, 1, 2),
       legend = c(paste("state", 1:2), "marginal"), bty = "n") 
...
for(j in 1:2) curve(delta1[j] * dnorm(x, beta[j, 1] + beta[j, 2], sigma[j]), 
                    add = T, col = color[j], lwd = 2)
curve(delta1[1] * dnorm(x, beta[1, 1] + beta[1, 2], sigma[1]) + 
        delta1[2] * dnorm(x, beta[2, 1] + beta[2, 2], sigma[2]),
      add = T, lwd = 2, lty = 2)
dev.off()
