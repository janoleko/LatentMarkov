...
beta0 = 0.02
mu0 = 0.001
qnorm(0.025, 0, sigma0 / sqrt(1 - phi0^2)) # 0.025 quantile of stationary distribution -> to find a good essential range
...
qnorm(0.025, 0, sigma / sqrt(1 - phi^2)) # 0.025 quantile of stationary distribution of AR1 process
...
lines(date[ind], (2 * beta * exp(Mean / 2) - 0.4)[ind], 
      type = "l", lwd = 1, col = "orange") # mean
...
lalpha = LaMa:::logalpha_cpp(allprobs, delta, 
                             array(Gamma, dim = c(m, m, nrow(allprobs) - 1)))
...
dforecast = function(x, delta, Gamma, lalpha, day, step = 1, mu, beta, bm){
  ...
  rowSums(matrix(phiT %*% (Gamma%^%step), nrow = nrow(allprobs), 
                 ncol = m, byrow = TRUE) * allprobs)
}
...
for(i in 1:length(testInd)){
  pred = dforecast(xseq, delta, Gamma, lalpha, day = testInd[i] - 1, 
                   mu = mu, beta = beta, bm = bm)
  pred = pred / sum(pred)
